...
const Waitlist = ({ isOpen, onClose }) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[50000] flex items-center justify-center p-[5%] mt-[60px]">
          ...
          <motion.div
            ...
            // Glassmorphism styling added here
            className="relative w-[60%] max-[700px]:w-[100%] bg-white/80 backdrop-blur-xl border border-white/30 rounded-[20px] px-[50px] py-[30px] max-[500px]:p-[30px] shadow-[0_8px_32px_0_rgba(0,0,0,0.3)] overflow-y-auto max-h-[90vh]"
          >
            {/* Close Button */}
            <button
              onClick={onClose}
              className="absolute top-5 right-5 text-gray-600 hover:text-black text-2xl font-bold transition-colors"
            >
              &times;
            </button>

            <h2 className="font-catamaran text-center text-[35px] max-[700px]:text-[30px] mb-[5px] leading-[1.2] text-black">
              Join The Waitlist
            </h2>
            <p className="font-raleway text-center text-[16px] max-[700px]:text-[15px] mb-[35px] text-gray-700">
              Be part of the excitement; Receive exclusive launch updates and
              notifications
            </p>
            ...
            <form onSubmit={handleSubmit} className="font-raleway w-full">
              <div className="w-full flex flex-wrap justify-between">
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="w-[48%] h-[50px] border border-white/50 bg-white/50 rounded-[5px] px-[15px] text-[16px] mb-[20px] focus:outline-none focus:ring-1 focus:ring-black max-[700px]:w-[100%] max-[500px]:text-[15px] max-[500px]:h-[40px]"
                />
                <input
                  type="text"
                  name="lastname"
              ...
              <input
                ...
                onChange={handleChange}
                required
                className="w-full h-[50px] border border-white/50 bg-white/50 rounded-[5px] px-[15px] text-[16px] mb-[20px] focus:outline-none focus:ring-1 focus:ring-black max-[500px]:text-[15px] max-[500px]:h-[40px]"
              />
              ...
              <select
                ...
                onChange={handleChange}
                required
                className="w-full h-[50px] border border-white/50 bg-white/50 rounded-[5px] px-[15px] text-[16px] mb-[20px] focus:outline-none focus:ring-1 focus:ring-black cursor-pointer max-[500px]:text-[15px] max-[500px]:h-[40px]"
              >
                <option value="">Select Location</option>
                {region.map((state, index) => (
                  <option key={index} value={state}>
              ...
              <button
                type="submit"
                disabled={loading}
                className="cursor-pointer w-full h-[55px] bg-black text-white rounded-[5px] text-[18px] font-semibold transition-transform active:scale-[0.98] disabled:opacity-50 max-[500px]:text-[15px] max-[500px]:h-[40px]"
              >
                {loading ? "Submitting Form..." : "Join Now"}
              </button>
